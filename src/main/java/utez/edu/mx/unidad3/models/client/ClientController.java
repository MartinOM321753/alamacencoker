...
public class ClientController {
    ...
    @GetMapping("")
    @Operation(summary = "Traer clientes", description = "Retorna todos los clientes registrados en el sistema")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Traer todos los clinetes",
                    content = {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Traer un cliente", description = "Retorna al cliente registrado en el sistema")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Traer un clinete",
                    content = {
                            @Content(mediaType = "application/json", schema = @Schema(implementation = APIResponse.class)),
            ...
            @ApiResponse(
                    responseCode = "500",
                    description = "No se pudo consultar al ciente",
                    content = {
                            @Content(mediaType = "application/json", schema = @Schema(implementation = APIResponse.class)),
    ...
    @PostMapping("/save")
    @Operation(summary = "Guardar al cliente", description = "Registar la informacion de un cliente")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "201",
            ...
            @ApiResponse(
                    responseCode = "500",
                    description = "No se pudo registar al ciente",
                    content = {
                            @Content(mediaType = "application/json", schema = @Schema(implementation = APIResponse.class)),
    ...
    public ResponseEntity<APIResponse> save(@RequestBody Client client) {
        APIResponse response = clientService.save(client);
        return new ResponseEntity<>(response, response.getStatus());
    }
